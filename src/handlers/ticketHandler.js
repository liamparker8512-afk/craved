...
async function closeTicket(interaction) {
  ...
  try {
    // Discord only returns up to 100 messages per call, so page backwards with `before`
    // until we've collected everything (capped to avoid runaway fetches on huge tickets).
    const MAX_MESSAGES = 2000;
    let allMessages = [];
    let lastId = null;
    while (allMessages.length < MAX_MESSAGES) {
      const batch = await interaction.channel.messages.fetch({ limit: 100, before: lastId || undefined });
