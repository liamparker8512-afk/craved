...
function applicationsPage(guild) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      ...
      `**Accept role:** ${cfg.applicationAcceptRoleId ? `<@&${cfg.applicationAcceptRoleId}>` : '_Not set_'}\n` +
  ...
  const rows = [
    ...
    new ActionRowBuilder().addComponents(
      new RoleSelectMenuBuilder().setCustomId('panel_role_appaccept').setPlaceholder('Set accept role').setMinValues(1).setMaxValues(1)
    ),
    new ActionRowBuilder().addComponents(
...
const ROLE_KEY_MAP = {
  ...
  staff: 'staffRoleId',
  mod: 'modRoleId',
  appaccept: 'applicationAcceptRoleId'
};
...
async function route(interaction) {
  ...
  if (id.startsWith('panel_role_')) {
    ...
    const page = shortKey === 'appaccept' ? 'applications' : 'roles';
    const { embed, components } = renderPage(interaction.guild, page);
    return interaction.update({ embeds: [embed], components });
  }
