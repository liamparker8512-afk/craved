...
module.exports = {
  ...
  async execute(interaction) {
    try {
      // This entire bot is guild-only — every handler below assumes interaction.guild exists.
      // Guard against DM-context interactions crashing on `.guild.id`.
      if (!interaction.guild) {
        if (interaction.isRepliable()) {
          await interaction.reply({ content: 'This bot only works inside a server, not in DMs.', ephemeral: true }).catch(() => {});
        }
        return;
      }
      ...
      // Application modal submissions (deny reason)
      if (interaction.isModalSubmit() && interaction.customId.startsWith('app_deny_modal_')) {
        const submissionId = interaction.customId.replace('app_deny_modal_', '');
        return applicationHandler.handleDenyModalSubmit(interaction, submissionId);
      }
      ...
      if (interaction.isButton()) {
        ...
        if (interaction.customId === 'application_apply') {
          if (!isFeatureEnabled(interaction.guild.id, 'applications')) {
            return interaction.reply({ embeds: [errorEmbed('Applications are currently disabled on this server.')], ephemeral: true });
          }
        ...
        if (interaction.customId.startsWith('app_accept_')) {
          ...
          const submissionId = interaction.customId.replace('app_accept_', '');
          return applicationHandler.handleAccept(interaction, submissionId);
        }
        if (interaction.customId.startsWith('app_deny_')) {
          if (!isFeatureEnabled(interaction.guild.id, 'applications')) {
            return interaction.reply({ embeds: [errorEmbed('Applications are currently disabled on this server.')], ephemeral: true });
          }
          const submissionId = interaction.customId.replace('app_deny_', '');
          return applicationHandler.handleDenyButton(interaction, submissionId);
        }
