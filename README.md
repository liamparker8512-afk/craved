# Dai Hood Remake — All-in-One Discord Bot

An all-in-one Discord bot built for a "Dai Hood" style Roblox game community, similar in spirit to bots like Ticket Tool, Greed, and Bleed. Built with **discord.js v14**, plain JSON file storage (no database server or native builds required), and organized into clean, extensible slash-command modules.

## Features

- **📝 Applications** (Appy-style)
  - Admin sets up a review channel, an optional auto-role for accepted applicants, and a list of custom questions
  - `/application panel` posts an **Apply Now** button in any channel (or users can just run `/application apply`)
  - Clicking it DMs the applicant each question **one at a time**, waiting up to 5 minutes per answer
  - On completion, a compiled Q&A embed is posted in the review channel with **Accept**/**Deny** buttons
  - **Accept** — DMs the applicant, optionally grants the configured role, marks the application reviewed
  - **Deny** — opens a reason box for the reviewer, then DMs the applicant that reason
  - `/application toggle` opens/closes applications; `/application question add/remove/list` manages the question set
...
- **💵 Economy** (Dai Hood themed "Cash") — balance, daily, work, beg, rob, pay, deposit/withdraw (bank), leaderboard, shop, buy
...
- **🧹 Basic Auto-Moderation** — deletes invite links and any words/phrases you configure via `/config bannedwords add|remove|list` (empty by default — nothing is banned out of the box). Staff/Admins are automatically exempt so they can share links etc.
...
- **⚙️ Central config command** — `/config welcome`, `/config leave`, `/config logs`, `/config adminrole`, `/config modrole`, `/config staffrole`, `/config suggestions`, `/config bannedwords add|remove|list`. (Feature on/off toggles, including auto-moderation, live in `/panel` instead.)
...
- **Applications page** — dropdown pickers for the review channel and accept role, plus an Open/Close toggle button. Question management still happens via `/application question add/remove/list` since typing questions doesn't suit a button UI.
- **Feature Toggles page** — one button per module (Tickets, Applications, Moderation, Economy, Leveling, Giveaways, Suggestions, Welcome, Leave, Auto-Moderation, Reaction Roles). Click to flip a feature on/off instantly — green = on, grey = off. Disabled modules immediately stop responding to their commands/events server-wide.
...
- The banned-word list is empty by default and fully configurable in-Discord — no code editing needed. Add words with `/config bannedwords add`, and turn Auto-Moderation on in `/panel`.
